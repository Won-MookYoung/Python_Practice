...
# infos=driver.find_elements(By.CSS_SELECTOR, 'basicList_inner__eY_mq')
...
#         link = info.find_element(By.CSS_SELECTOR, 'a>img').get_attribute('src')                                                               
        
#         print(f'{i}번째 상품',product.text, price.text, link,sep='\n')
#         i+=1
#     except:
#         continue
...
for i in range(1,int(page_num)+1):
    if i >=2:
        ...
        infos=driver.find_elements(By.CSS_SELECTOR, '.basicList_item__2XT81')
        ...
        for info in infos:
            ...
            product= info.find_element(By.CSS_SELECTOR, '.basicList_link__1MaTN')
            price = info.find_element(By.CSS_SELECTOR, '.price_num__2WUXn')   
            img_src = info.find_element(By.CSS_SELECTOR, 'a>img')                                                               

            print(f'{a}번째 상품',product.text, price.text, img_src.get_attribute('src'),sep='\n')
            a+=1

            ws.append([product.text, img_src.get_attribute('src'), price.text])
